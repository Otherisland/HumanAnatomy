...
import static com.bn.hk.humananatomy.util.ShaderUtil.createProgram;


//纹理三角形
public class TexDrawert

{
    ...
    public void initShader(ShowView mv)
    {
            ...
            //加载顶点着色器的脚本内容
            mVertexShader= ShaderUtil.loadFromAssetsFile(Constant.shaders+"vertex_load2d.sh", mv.getResources());
            //加载片元着色器的脚本内容
            mFragmentShader= ShaderUtil.loadFromAssetsFile(Constant.shaders+"frag_load2d.sh", mv.getResources());
            //基于顶点着色器与片元着色器创建程序
            ...
            //当前进度的引用
            mLoadHandle=GLES30.glGetUniformLocation(mProgram, "xPosition");

    }

    public void drawSelf(int texId,float x,float y,int width,int height)
    {
        //开启混合
        ...
        GLES30.glUniform1f(mLoadHandle, Constant.loadPosition); //加载当前进度
